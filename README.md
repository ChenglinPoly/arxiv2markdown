...
- 🚀 **并行处理**: 支持多线程并行处理，大幅提升转换效率
- 📦 **智能解析**: 自动识别主TeX文件，处理复杂的LaTeX项目结构
- 🖼️ **图片提取**: 自动提取并保存图片资源，保持文档完整性
- 🔧 **容错机制**: robust错误处理，失败文件自动移至专门目录并记录详细日志
- ⚡ **高效转换**: 基于Pandoc的高质量LaTeX到Markdown转换
- 📝 **详细统计**: 提供处理时间、成功率等详细统计报告
...
查看 `failed/fail.log` 获取详细错误信息。

### Q: 如何调整处理性能？
A: 
- 增加worker数量：`-w 8`（建议不超过CPU核心数）
- 确保有足够的磁盘空间用于临时文件
- 对于特别复杂的文件，可以考虑单独手动处理

### Q: 处理大量文件时如何监控进度？
A: 并行处理脚本会实时显示进度信息，包括当前处理的文件和完成情况。

### Q: 输出的Markdown质量如何？
A: 系统基于Pandoc进行转换，能够很好地保持：
- 数学公式（保留LaTeX格式）
- 图片引用和链接
- 文档结构和格式
- 参考文献
...
- **智能主文件检测**: 支持多种主TeX文件命名规范
- **并发安全**: 使用线程锁确保文件操作安全
- **内存优化**: 临时文件自动清理，避免磁盘占用
- **错误隔离**: 单个文件失败不影响其他文件处理
- **详细日志**: 完整的处理日志记录，便于问题排查
